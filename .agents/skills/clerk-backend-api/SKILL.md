...
Before doing anything outside the FAST PATH, fetch the available spec versions and tags by running (in terminal from the workspace root):

```bash
bash .agents/skills/clerk-backend-api/scripts/api-specs-context.sh
```
...
Determine the active mode based on the user's message:
...
Print the following usage guide to the user:

```
Browse
  "list all Clerk API tags"                       — list all available API tags
  "show endpoints for the Users tag"              — browse endpoints for a tag
  "show Users tag using version 2025-11-10.yml"   — browse using a specific version

Execute
  "fetch all users from Clerk"                    — list users
  "get user john_doe"                             — natural language works too
  "create a Clerk invitation"                     — create an invitation

Inspect
  "show schema for GET /users without executing"  — view endpoint schema only
  "inspect POST /invitations"                     — view request/response details

Options
  --admin                            — bypass scope restrictions for write/delete
  --version [date]                   — use a specific spec version
```
...
If using a non-latest version, fetch tags for that version (run in terminal from the skill directory):

```bash
curl -s https://raw.githubusercontent.com/clerk/openapi-specs/main/bapi/${version_name} | node .agents/skills/clerk-backend-api/scripts/extract-tags.js
```
...
Fetch all endpoints for the identified tag (run in terminal from the workspace root):

```bash
curl -s https://raw.githubusercontent.com/clerk/openapi-specs/main/bapi/${version_name} | bash .agents/skills/clerk-backend-api/scripts/extract-tag-endpoints.sh "${tag_name}"
```
...
Extract the full endpoint definition (run in terminal from the workspace root):

```bash
curl -s https://raw.githubusercontent.com/clerk/openapi-specs/main/bapi/${version_name} | bash .agents/skills/clerk-backend-api/scripts/extract-endpoint-detail.sh "${path}" "${method}"
```
...
- Load `clerk-setup` via `read_file` at `d:\my-Project\duolingo-clone-project\.agents\skills\clerk-setup\SKILL.md` — Initial Clerk install
- Load `clerk-webhooks` via `read_file` at `d:\my-Project\duolingo-clone-project\.agents\skills\clerk-webhooks\SKILL.md` — Real-time event sync
- `clerk-orgs` — not installed locally; see https://clerk.com/docs/organizations/overview
